...
const MOCK_SETTINGS = {
  addr: ":8080",
  theme: "light",
  book: { path: "./books/performance.bin" },
  engines: {
    stockfish: {
      enabled: true,
      ...
      difficulty: "medium",
      forPlay: true,
      options: { Threads: "2", Hash: "128", MultiPV: "3", "Skill Level": "5", UCI_LimitStrength: "true", UCI_Elo: "1600" },
    },
    maia: { enabled: false, path: "/opt/lc0/lc0", difficulty: "maia-default", forPlay: true, options: { Threads: "1" } },
  },
  ui: { defaultAnalysisEngine: "stockfish", defaultPlayEngine: "stockfish", showBook: true, showAnalysis: true },
};
const MOCK_SESSION = {
  id: "mock-session",
  moves: [],
  redoMoves: [],
  snapshot: { fen: FEN.start, turn: "w", status: "in_progress", outcome: "", method: "" },
  mode: { playingAgainstEngine: false, engineName: "", humanColor: "white", difficulty: "medium" },
};
...
function loadStoredTheme() {
  try { return localStorage.getItem(STORAGE_KEYS.theme) || null; } catch { return null; }
}
...
function saveStoredViewState(state) {
  try { localStorage.setItem(STORAGE_KEYS.view, JSON.stringify(state)); } catch {}
}
...
function mockApi(path, opt = {}) {
  ...
  if (path.includes("/legal")) {
    ...
    return Promise.resolve({ moves: [`${sq.slice(0, 2)}e4`, `${sq.slice(0, 2)}e3`] });
  }
  if (path.includes("/recommendations")) {
    return Promise.resolve({
      ...
      engine: [
        { engine: currentSettings.ui.defaultAnalysisEngine, bestMove: "e2e4", cached: false, raw: "mock backend unavailable" },
      ],
    });
  }
...
`);
  ...
  if (path === "/api/replay/load" && method === "POST") {
    ...
    return Promise.resolve({ headers: p.headers, frames: [{ fen: currentGame.snapshot.fen, moveLabels: p.moves }] });
  }
  return Promise.resolve(structuredClone(currentGame));
}
...
function collectSettings() {
  return {
    ...
    book: { path: els.cfgBookPath.value || "" },
    engines: {
      stockfish: {
...
function renderPGNDetails(parsed) {
  ...
  [["pgnEvent", h.Event || ""], ["pgnSite", h.Site || ""], ["pgnDate", h.Date || ""], ["pgnRound", h.Round || ""], ["pgnWhite", h.White || ""], ["pgnBlack", h.Black || ""], ["pgnResult", h.Result || ""], ["pgnEco", h.ECO || ""]].forEach(([id, val]) => {
    els[id].value = val;
  });
...
function clearPGNDetails() { renderPGNDetails({ headers: {}, moves: [] }); }

async function loadReplay(raw) {
  const parsed = parsePGN(raw);
  renderPGNDetails(parsed);
  currentReplay = await api("/api/replay/load", { method: "POST", body: raw });
  replayIndex = 0;
  renderReplayFrame();
  showPage("pageMain");
}
...
function renderReplayFrame() {
  ...
  els.replayInfo.textContent = JSON.stringify({ index: replayIndex, headers: currentReplay.headers, frame }, null, 2);
}

function validateFEN(fen) {
  const result = { ok: false, error: "" };
  const trimmed = String(fen).trim();
  if (!trimmed) { result.error = "Enter a full 6-part FEN."; return result; }
  const parts = trimmed.split(/\s+/);
  if (parts.length !== 6) { result.error = "FEN must contain 6 space-separated fields."; return result; }
  const [boardPart, side, castling, ep, halfmove, fullmove] = parts;
  const ranks = boardPart.split("/");
  if (ranks.length !== 8) { result.error = "Board field must contain 8 ranks."; return result; }
  let WK = 0; let BK = 0;
  for (const rank of ranks) {
    let count = 0;
    for (const ch of rank) {
      if (/[1-8]/.test(ch)) { count += Number(ch); }
      else if (/[pnbrqkPNBRQK]/.test(ch)) { count += 1; if (ch === "K") WK += 1; if (ch === "k") BK += 1; }
      else { result.error = `Invalid board character: ${ch}`; return result; }
    }
    if (count !== 8) { result.error = "Each rank must describe exactly 8 squares."; return result; }
  }
  if (WK !== 1 || BK !== 1) { result.error = "FEN must contain exactly one white king and one black king."; return result; }
  if (!/^[wb]$/.test(side)) { result.error = "Side-to-move field must be w or b."; return result; }
  if (!(castling === "-" || /^[KQkq]+$/.test(castling))) { result.error = "Castling field must be - or a combination of KQkq."; return result; }
  if (!(ep === "-" || /^[a-h][36]$/.test(ep))) { result.error = "En-passant field must be - or a square like e3 or d6."; return result; }
  if (!/^\d+$/.test(halfmove)) { result.error = "Halfmove clock must be a non-negative integer."; return result; }
  if (!/^\d+$/.test(fullmove) || Number(fullmove) < 1) { result.error = "Fullmove number must be an integer >= 1."; return result; }
  result.ok = true; result.error = "FEN looks valid."; return result;
}
...
function loadFENIntoBoard(fen) {
  currentGame.snapshot = { ...currentGame.snapshot, fen, turn: fen.includes(" b ") ? "b" : "w", status: "in_progress" };
  currentGame.moves = [];
...
async function startAnalysisFromFEN(fen) {
  currentGame = await api("/api/game/new", { method: "POST", body: JSON.stringify({ fen }) });
  currentReplay = null;
  replayIndex = 0;
  renderSession(currentGame);
  clearPGNDetails();
...
async function playMove(move) {
  currentGame = await api(`/api/game/${currentGame.id}/move`, { method: "POST", body: JSON.stringify({ move }) });
  renderSession(currentGame);
  await loadRecommendations();
}
...
async function newVsEngine(config = null) {
  ...
  currentGame = await api("/api/game/new-vs-engine", { method: "POST", body: JSON.stringify(payload) });
  currentReplay = null;
  replayIndex = 0;
  renderSession(currentGame);
  clearPGNDetails();
...
function renderSession(session) {
  ...
  els.gameInfo.textContent = JSON.stringify({ id: session.id, fen: session.snapshot?.fen, turn: session.snapshot?.turn, status: session.snapshot?.status, mode: session.mode, backend: backendAvailable ? "connected" : "mock" }, null, 2);
}
...
function bindEvents() {
  ...
  els.menuSettings.onclick = () => { renderArrangePanels(); showPage("pageSettings"); };
  ...
  els.undoBtn.onclick = async () => {
    currentGame = await api(`/api/game/${currentGame.id}/undo`, { method: "POST" });
    renderSession(currentGame);
    await loadRecommendations();
  };
  els.redoBtn.onclick = async () => {
    currentGame = await api(`/api/game/${currentGame.id}/redo`, { method: "POST" });
    renderSession(currentGame);
    await loadRecommendations();
  };
  els.recommendBtn.onclick = () => loadRecommendations();
  els.engineMoveBtn.onclick = async () => {
    currentGame = await api(`/api/game/${currentGame.id}/engine-move`, { method: "POST" });
    renderSession(currentGame);
    await loadRecommendations();
  };
  ...
  els.pgnFileLoadBtn.onclick = async () => {
    if (!selectedPGNFile) { alert("No PGN file selected."); return; }
